...
"""
def getGraph(coef_matrix, enhance_matrix, K, L):
    sim_sh = 1.
    for i in range(len(enhance_matrix)):
        enhance_matrix[i][i] = sim_sh * max(enhance_matrix[i])

    data = enhance_matrix.reshape(-1)
    data = np.sort(data)
    ...
    propor = len(enhance_matrix[enhance_matrix <= candi_sh])/(len(enhance_matrix)**2)
    propor = 1 - propor
    thres = np.sort(enhance_matrix)[:, -int(len(enhance_matrix)*propor)]
    coef_matrix.T[enhance_matrix.T <= thres] = 0


    N = len(coef_matrix)
    
    up_K = np.sort(coef_matrix)[:,-K]
    
    mat_K = np.zeros(coef_matrix.shape)
    mat_K.T[coef_matrix.T >= up_K] = 1
    
    thres_L = np.sort(coef_matrix.flatten())[-int(((N*N)//(1//(L+1e-8))))]
    mat_K.T[coef_matrix.T < thres_L] = 0
